...
<head>
    ...
    <title>Edit Group Booking - {{ config('app.name') }}</title>
...
<body>
    <div class="dashboard-container">
        ...
        <div class="main-content">
            <div class="container">
                <div class="header">
                    <h1><i class="fas fa-edit"></i> Edit Group Booking</h1>
                    <p>Update the information for "{{ $groupBooking->name }}"</p>
                </div>
                ...
                <form action="{{ route('tenant.group-bookings.update', $groupBooking->id) }}" method="POST" class="form-container">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" id="name" name="name" class="form-control" 
                                       value="{{ old('name', $groupBooking->name) }}" required 
                                ...
                                <select id="property_id" name="property_id" class="form-control" required>
                                    ...
                                    @foreach($properties as $property)
                                        <option value="{{ $property->id }}" 
                                            {{ old('property_id', $groupBooking->property_id) == $property->id ? 'selected' : '' }}>
                                            {{ $property->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="total_rooms" name="total_rooms" class="form-control" 
                                       value="{{ old('total_rooms', $groupBooking->total_rooms) }}" required min="1">
                                @error('total_rooms')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="status" name="status" class="form-control" required>
                                    <option value="PENDING" {{ old('status', $groupBooking->status) === 'PENDING' ? 'selected' : '' }}>Pending</option>
                                    <option value="CONFIRMED" {{ old('status', $groupBooking->status) === 'CONFIRMED' ? 'selected' : '' }}>Confirmed</option>
                                    <option value="CANCELLED" {{ old('status', $groupBooking->status) === 'CANCELLED' ? 'selected' : '' }}>Cancelled</option>
                                    <option value="COMPLETED" {{ old('status', $groupBooking->status) === 'COMPLETED' ? 'selected' : '' }}>Completed</option>
                                </select>
                                @error('status')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="date" id="arrival_date" name="arrival_date" class="form-control" 
                                       value="{{ old('arrival_date', $groupBooking->arrival_date?->format('Y-m-d')) }}" required>
                                @error('arrival_date')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" id="departure_date" name="departure_date" class="form-control" 
                                       value="{{ old('departure_date', $groupBooking->departure_date?->format('Y-m-d')) }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <select id="leader_guest_id" name="leader_guest_id" class="form-control">
                                    ...
                                    @if($groupBooking->leaderGuest)
                                        <option value="{{ $groupBooking->leaderGuest->id }}" selected>
                                            {{ $groupBooking->leaderGuest->full_name }}
                                        </option>
                                    @endif
                                </select>
                                ...
                                <select id="corporate_account_id" name="corporate_account_id" class="form-control">
                                    ...
                                    @foreach($corporateAccounts as $corporate)
                                        <option value="{{ $corporate->id }}" 
                                            {{ old('corporate_account_id', $groupBooking->corporate_account_id) == $corporate->id ? 'selected' : '' }}>
                                            {{ $corporate->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-grid single">
                            <div class="form-group">
                                ...
                                <textarea id="notes" name="notes" class="form-control" 
                                          placeholder="Any special requests, requirements, or important information...">{{ old('notes', $groupBooking->notes) }}</textarea>
                                @error('notes')
                                    <span class="error-message">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="form-actions">
                        <a href="{{ route('tenant.group-bookings.show', $groupBooking->id) }}" class="btn btn-secondary">
                            <i class="fas fa-times"></i> Cancel
                        </a>
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> Update Group Booking
                        </button>
                    </div>
                </form>
            </div>
